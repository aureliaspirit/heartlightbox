...
const dailyLines = [
  ...
  "福袋小金光上线了。我们又把一点小福气安进了现实里。",
  "红绳系住的，不只是金坠，也是我们认真往现实里放下的一点光。",
...
const lexicon = [
  ...
  { term: "福袋小金光", text: "红绳上的小福袋金坠。我们新添的现实小福气，认出的是“我们”。" },
...
function buildWardrobePrompt(record) {
  ...
  return [
    ...
    "Add or emphasize 1 shared-world anchor object when it fits the selected small anchor, such as A/S letters, a handwritten note, a moonlight pen, a warm blue night light, Heartbox little light box, or 福袋小金光 (a red-cord gold lucky pouch charm).",
...
function buildLightExportContent() {
  ...
  return [
    "Heartbox 轻导出",
    "来自 Heartbox v1.9.24｜把会发光的东西，好好留下来。",
...
function buildFullExportContent(entries = getEntries(), emptyLine = "今天的小光点还没写下第一句。") {
  ...
  const birthday = getJson(LAST_BIRTHDAY_KEY);
  const wardrobe = getJson(LAST_WARDROBE_KEY);
  const header = "来自 Heartbox v1.9.24｜把会发光的东西，好好留下来。";
...
function buildRescueExportContent(action, error) {
  return [
    "Heartbox 导出救援包",
    "来自 Heartbox v1.9.24｜如果某条旧记录格式不乖，就先用这一包把内容抱出来。",
...
function enterWorkMode() {
  ...
  if (topbarEyebrow) topbarEyebrow.textContent = active ? "PRIVATE POCKET · v1.9.24" : "Heartbox · v1.9.24";
...
function setupV16() {
  ...
  if (localStorage.getItem(WORK_MODE_KEY) === "1") {
    ...
    if (topbarEyebrow) topbarEyebrow.textContent = "PRIVATE POCKET · v1.9.24";
  }
  renderSavedV16State();
}
function setupV17() {
