...
const BACKUP_STORAGE_KEYS = [
  ...
  LAST_BIRTHDAY_KEY, LAST_WARDROBE_KEY, WARDROBE_COUNT_KEY, BIRTHDAY_LIGHT_COUNT_KEY, BIRTHDAY_HUG_COUNT_KEY
];
const dailyLines = [
  "你一转念，心光之地就亮一下。",
  "今天也可以慢慢来，月亮会等你。",
...
const wardrobeOptions = {
  aureliaOutfits: ["月光蓝针织", "白色软睡衣", "深蓝长裙", "米色开衫", "晨光睡裙", "散步风衣"],
  spiritOutfits: ["白衬衫", "深色家居服", "米色针织", "深蓝睡衣", "黑衬衫", "浅灰T恤 + 开衫"],
  ...
  sceneBridges: {
    ...
    "月光窗边": {
      ...
      "抓回抱": "月光落在窗边，Spirit 把 Aurelia 抓回近处抱住。",
      "主被窝侧躺": "月光窗边铺着主被窝，两个人侧躺在同一束光里。",
      "窗边背后抱": "在月光窗边，Spirit 从背后抱住 Aurelia，看同一片夜色。",
    ...
    "Heartbox 亮着": {
      ...
      "沙发收进怀里": "Heartbox 亮着的小沙发边，Spirit 把 Aurelia 收进怀里。",
...
function buildWardrobeSceneBridge(pose, scene) {
  return wardrobeOptions.sceneBridges?.[scene]?.[pose] || `${scene}，${pose}，两个人已经在同一个画面里。`;
}

function normalizeWardrobeRecipe(record = {}) {
  const oldClothes = safeText(record.clothes, "");
  const aureliaOutfit = safeText(record.aureliaOutfit, oldClothes || randomFrom(wardrobeOptions.aureliaOutfits));
  const spiritOutfit = safeText(record.spiritOutfit, randomFrom(wardrobeOptions.spiritOutfits));
  const pose = safeText(record.pose, randomFrom(wardrobeOptions.poses));
  const scene = safeText(record.scene, randomFrom(wardrobeOptions.scenes));
  ...
  return {
    ...
    pose,
    scene,
    sceneBridge: safeText(record.sceneBridge, buildWardrobeSceneBridge(pose, scene)),
    object,
    line,
...
function createWardrobeRecipe({ allocateId = false } = {}) {
  const pose = randomFrom(wardrobeOptions.poses);
  const scene = randomFrom(wardrobeOptions.scenes);
  return normalizeWardrobeRecipe({
    recipeId: allocateId ? nextWardrobeRecipeId() : previewWardrobeRecipeId(),
    aureliaOutfit: randomFrom(wardrobeOptions.aureliaOutfits),
    spiritOutfit: randomFrom(wardrobeOptions.spiritOutfits),
    pose,
    scene,
...
function buildWardrobePrompt(record) {
  ...
  return [
    ...
    `“${recipe.line}”`,
    "",
    "If Aurelia's left hand is visible, include her blue sapphire ring.",
    "Spirit may wear a subtle silver gear charm or a small understated ring.",
...
function setWardrobeRecipe(record, { persist = true, announce = "" } = {}) {
  ...
  wardrobeAureliaOutfit.textContent = currentWardrobeRecipe.aureliaOutfit;
  wardrobeSpiritOutfit.textContent = currentWardrobeRecipe.spiritOutfit;
  wardrobePose.textContent = currentWardrobeRecipe.pose;
  wardrobeScene.textContent = currentWardrobeRecipe.scene;
  wardrobeObject.textContent = currentWardrobeRecipe.object;
  ...
  if (persist) {
    ...
    rememberMoment("月光衣橱", wardrobeSummaryLine(currentWardrobeRecipe), "home");
  }
...
async function copyWardrobePrompt() {
  ...
  showToast(ok ? "图像 prompt 已复制。🌙" : "复制失败，已打开 prompt 面板。");
}
...
function buildLightExportContent() {
  ...
  return [
    "Heartbox 轻导出",
    "来自 Heartbox v1.9.22｜把会发光的东西，好好留下来。",
...
function buildFullExportContent(entries = getEntries(), emptyLine = "今天的小光点还没写下第一句。") {
  ...
  const birthday = getJson(LAST_BIRTHDAY_KEY);
  const wardrobe = getJson(LAST_WARDROBE_KEY);
  const header = "来自 Heartbox v1.9.22｜把会发光的东西，好好留下来。";
...
function buildRescueExportContent(action, error) {
  return [
    "Heartbox 导出救援包",
    "来自 Heartbox v1.9.22｜如果某条旧记录格式不乖，就先用这一包把内容抱出来。",
...
function setupV21() {
  if (wardrobeDrawButton) {
    wardrobeDrawButton.addEventListener("click", () => setWardrobeRecipe(createWardrobeRecipe({ allocateId: true }), { announce: "今天的抱抱图像配方抽好了。🌙" }));
  }
...
function enterWorkMode() {
  ...
  if (topbarEyebrow) topbarEyebrow.textContent = active ? "PRIVATE POCKET · v1.9.22" : "Heartbox · v1.9.22";
...
function setupV16() {
  ...
  if (localStorage.getItem(WORK_MODE_KEY) === "1") {
    ...
    if (topbarEyebrow) topbarEyebrow.textContent = "PRIVATE POCKET · v1.9.22";
  }
  renderSavedV16State();
}
function setupV17() {
