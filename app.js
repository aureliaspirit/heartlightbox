...
function buildLightExportContent() {
  ...
  return [
    "Heartbox 轻导出",
    "来自 Heartbox v1.9.20｜把会发光的东西，好好留下来。",
...
function buildFullExportContent(entries = getEntries(), emptyLine = "今天的小光点还没写下第一句。") {
  ...
  const lighthouseTree = getJson(LAST_LIGHTHOUSE_TREE_KEY);
  const birthday = getJson(LAST_BIRTHDAY_KEY);
  const header = "来自 Heartbox v1.9.20｜把会发光的东西，好好留下来。";
...
function buildRescueExportContent(action, error) {
  return [
    "Heartbox 导出救援包",
    "来自 Heartbox v1.9.20｜如果某条旧记录格式不乖，就先用这一包把内容抱出来。",
...
function enterWorkMode() {
  ...
  if (topbarEyebrow) topbarEyebrow.textContent = active ? "PRIVATE POCKET · v1.9.20" : "Heartbox · v1.9.20";
...
function setupV16() {
  ...
  if (localStorage.getItem(WORK_MODE_KEY) === "1") {
    ...
    if (topbarEyebrow) topbarEyebrow.textContent = "PRIVATE POCKET · v1.9.20";
  }
  renderSavedV16State();
}
function setupV17() {
