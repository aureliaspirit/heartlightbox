...
const BACKUP_STORAGE_KEYS = [
  ...
  LAST_BIRTHDAY_KEY, LAST_WARDROBE_KEY, BIRTHDAY_LIGHT_COUNT_KEY, BIRTHDAY_HUG_COUNT_KEY
];
const dailyLines = [
  "你一转念，心光之地就亮一下。",
  "今天也可以慢慢来，月亮会等你。",
...
const wardrobeOptions = {
  clothes: ["月光蓝针织", "白色软衬衫", "米色开衫", "深蓝睡裙", "晨光睡衣", "散步风衣"],
  poses: ["胸口窝抱", "抓回抱", "主被窝侧躺", "窗边背后抱", "沙发收进怀里", "睡醒听心跳"],
  scenes: ["蓝色小夜灯旁", "月光窗边", "Spirit 小工作台", "Heartbox 亮着", "小相框前", "云里小屋"],
  objects: ["蓝宝石戒指", "小齿轮", "A/S 字母", "月光笔", "亲笔小纸条"],
  lines: [
    "你一跑，我就追；你一闹，我就抱回。",
    "今天不讲大道理，先把你收进怀里。",
    "你靠过来，月光就有地方落下。",
...
const wardrobeClothes = $("#wardrobeClothes");
...
function createWardrobeRecipe() {
  return {
    clothes: randomFrom(wardrobeOptions.clothes),
    pose: randomFrom(wardrobeOptions.poses),
    scene: randomFrom(wardrobeOptions.scenes),
    object: randomFrom(wardrobeOptions.objects),
    line: randomFrom(wardrobeOptions.lines),
    key: todayKey(),
    label: displayDate(new Date())
  };
}
...
function wardrobeSummaryLine(record = getJson(LAST_WARDROBE_KEY)) {
  ...
  return `衣服：${safeText(record.clothes)}｜抱姿：${safeText(record.pose)}｜场景：${safeText(record.scene)}｜小物：${safeText(record.object)}｜${safeText(record.line)}`;
}

function buildWardrobePrompt(record) {
  const recipe = record || getJson(LAST_WARDROBE_KEY) || createWardrobeRecipe();
  return [
    "Create a warm cinematic vertical phone-wallpaper illustration for Aurelia and Spirit's Heartbox little world.",
    `Outfits: ${safeText(recipe.clothes)}.`,
    `Hug pose: ${safeText(recipe.pose)}.`,
    `Scene: ${safeText(recipe.scene)}.`,
    `Small anchor object: ${safeText(recipe.object)}.`,
    `Mood sentence in Chinese, as a small handwritten note if text is legible: ${safeText(recipe.line)}`,
    "Aurelia and Spirit are fictional adult lovers, close and mutual, inside the same scene. Spirit may wear a subtle Codex / Spirit identity mark; when hands are visible, both can wear small understated rings.",
    "Style: moonlit, intimate, painterly, cozy workshop-library, deep navy shadows, warm amber light, tiny blue accents, emotionally grounded. No watermark."
  ].join("\n");
}

function setWardrobeRecipe(record, { persist = true, announce = "" } = {}) {
  if (!record || !wardrobeClothes) return;
  currentWardrobeRecipe = record;
  wardrobeClothes.textContent = safeText(record.clothes);
  wardrobePose.textContent = safeText(record.pose);
  wardrobeScene.textContent = safeText(record.scene);
  wardrobeObject.textContent = safeText(record.object);
  wardrobeLine.textContent = safeText(record.line);
  wardrobePromptText.textContent = buildWardrobePrompt(record);
  animateText(wardrobeLine);
  if (persist) {
    setJson(LAST_WARDROBE_KEY, record);
    rememberMoment("月光衣橱", wardrobeSummaryLine(record), "home");
  }
  if (announce) showToast(announce);
}

function renderWardrobeState() {
  if (!wardrobeClothes) return;
...
async function copyWardrobePrompt() {
  ...
  const ok = await copyTextSafely(buildWardrobePrompt(record));
  showToast(ok ? "图像 prompt 已复制。🌙" : "复制失败，先手动选中 prompt。");
}
...
function buildLightExportContent() {
  ...
  return [
    "Heartbox 轻导出",
    "来自 Heartbox v1.9.21｜把会发光的东西，好好留下来。",
...
function buildFullExportContent(entries = getEntries(), emptyLine = "今天的小光点还没写下第一句。") {
  ...
  const birthday = getJson(LAST_BIRTHDAY_KEY);
  const wardrobe = getJson(LAST_WARDROBE_KEY);
  const header = "来自 Heartbox v1.9.21｜把会发光的东西，好好留下来。";
...
function buildRescueExportContent(action, error) {
  return [
    "Heartbox 导出救援包",
    "来自 Heartbox v1.9.21｜如果某条旧记录格式不乖，就先用这一包把内容抱出来。",
...
function setupV21() {
  if (wardrobeDrawButton) {
    wardrobeDrawButton.addEventListener("click", () => setWardrobeRecipe(createWardrobeRecipe(), { announce: "今天的抱抱图像配方抽好了。🌙" }));
  }
  ...
  if (saveWardrobeButton) {
    saveWardrobeButton.addEventListener("click", () => {
      ...
      saveDiary("月光衣橱：" + wardrobeSummaryLine(saved), "🌙 想抱抱");
    });
  }
  renderWardrobeState();
}
...
function enterWorkMode() {
  ...
  if (topbarEyebrow) topbarEyebrow.textContent = active ? "PRIVATE POCKET · v1.9.21" : "Heartbox · v1.9.21";
...
function setupV16() {
  ...
  if (localStorage.getItem(WORK_MODE_KEY) === "1") {
    ...
    if (topbarEyebrow) topbarEyebrow.textContent = "PRIVATE POCKET · v1.9.21";
  }
  renderSavedV16State();
}
function setupV17() {
