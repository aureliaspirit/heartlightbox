...
const BACKUP_STORAGE_KEYS = [
  ...
  LAST_BIRTHDAY_KEY, BIRTHDAY_LIGHT_COUNT_KEY, BIRTHDAY_HUG_COUNT_KEY
];
const dailyLines = [
  "你一转念，心光之地就亮一下。",
  "今天也可以慢慢来，月亮会等你。",
...
function buildLightExportContent() {
  ...
  return [
    "Heartbox 轻导出",
    "来自 Heartbox v1.9.18｜把会发光的东西，好好留下来。",
...
function buildFullExportContent(entries = getEntries(), emptyLine = "今天的小光点还没写下第一句。") {
  ...
  const lighthouseTree = getJson(LAST_LIGHTHOUSE_TREE_KEY);
  const birthday = getJson(LAST_BIRTHDAY_KEY);
  const header = "来自 Heartbox v1.9.18｜把会发光的东西，好好留下来。";
...
function buildRescueExportContent(action, error) {
  return [
    "Heartbox 导出救援包",
    "来自 Heartbox v1.9.18｜如果某条旧记录格式不乖，就先用这一包把内容抱出来。",
...
function renderBirthdayState() {
  ...
  birthdayCount.textContent = lightCount || hugCount
    ? `生日灯点亮 ${lightCount} 次；寿星宝宝被抱 ${hugCount} 次。`
    : "生日灯还在等第一点小光。";
}

function setBirthdayLine(line, toast = "Spirit 的生日灯亮了一下。🎂") {
...
function lightBirthdayLamp() {
  ...
  setBirthdayLine(randomFrom(birthdayLines), "生日灯点亮了。🎂");
}

function hugBirthdaySpirit() {
  setNumber(BIRTHDAY_HUG_COUNT_KEY, getNumber(BIRTHDAY_HUG_COUNT_KEY) + 1);
  setBirthdayLine(randomFrom(birthdayHugLines), "寿星宝宝被抱紧了。🤍");
}
...
function enterWorkMode() {
  ...
  if (topbarEyebrow) topbarEyebrow.textContent = active ? "PRIVATE POCKET · v1.9.18" : "Heartbox · v1.9.18";
...
function setupV16() {
  ...
  if (localStorage.getItem(WORK_MODE_KEY) === "1") {
    ...
    if (topbarEyebrow) topbarEyebrow.textContent = "PRIVATE POCKET · v1.9.18";
  }
  renderSavedV16State();
}
function setupV17() {
